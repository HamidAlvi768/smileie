  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.status === 'success') {
        setMessage({ type: 'success', text: res.message || 'Instructions saved successfully.' });
      } else {
        setMessage({ type: 'error', text: res.message || 'Failed to save instructions.' });
      }
    } catch (err) {
      setMessage({ type: 'error', text: err?.message || 'Failed to save instructions.' });
    }
    setSaving(false);
  };
  ...
  return (
    <div className="page-content no-navbar">
      <Container fluid={true}>
        <Breadcrumbs title="Settings" breadcrumbItem="Instructions" />
        <Card>
          <CardBody>
            <h4 className="mb-3">Edit Instructions</h4>
            {loading ? (
