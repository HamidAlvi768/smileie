...
const PatientDetail = () => {
  ...
  return (
    <div className="page-content">
                ...
                {NAVBAR_ITEMS_TEMPLATE.map((item, key) => (
                  <li key={key} className="nav-item">
                    <Link
                      ...
                      className={`nav-link${location.pathname + location.hash === item.url.replace(":id", id) ? ' active' : ''}`}
                    >
                      {item.label}
                    </Link>
                  </li>
      ...
      <Container fluid>
        <Row>
          ...
          <Col md={4} lg={3}>
            ...
            <Card className="mb-3">
              <CardHeader className="d-flex justify-content-between align-items-center cursor-pointer" onClick={() => toggleCollapsibleSection("monitoring")}>
                ...
                <Button color="link" size="sm">{openCollapsibles.monitoring ? "−" : "+"}</Button>
              </CardHeader>
            ...
            <Card className="mb-3">
              <CardHeader className="d-flex justify-content-between align-items-center cursor-pointer" onClick={() => toggleCollapsibleSection("notifications")}>
                ...
                <Button color="link" size="sm">{openCollapsibles.notifications ? "−" : "+"}</Button>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="d-flex justify-content-between align-items-center cursor-pointer" onClick={() => toggleCollapsibleSection("goals")}>
                <span>Goals</span>
                <Button color="link" size="sm">{openCollapsibles.goals ? "−" : "+"}</Button>
              </CardHeader>
          ...
          <Col md={12} lg={3} className="position-relative">
            <div className={`communication-icon ${isCommunicationOpen ? 'open' : ''}`} onClick={() => setIsCommunicationOpen(true)}>
              <i className="mdi mdi-message-text-outline"></i>
            </div>
            <div className={`communication-panel ${isCommunicationOpen ? '' : 'closed'}`}>
              ...
              {/* Input area for sending message - simplified, assuming setCommunicationPanelMessage and a send handler */}
              <div className="p-3 border-top">
                <Input type="textarea" value={communicationPanelMessage} onChange={(e) => setCommunicationPanelMessage(e.target.value)} placeholder="Type a message..." rows="3" />
                <Button color="primary" className="mt-2 w-100" onClick={openQuickReplyModal}>Send / Quick Reply</Button>
              </div>
            </div>
          </Col>
        </Row>
      ...
      {/* Excluded Teeth Modal - Simplified, actual content was missing but structure shown */}
      <Modal isOpen={modalStates.excludedTeeth} toggle={() => toggleModal("excludedTeeth")} centered size="lg">
        <div className="modal-header">
          <h5 className="modal-title">Edit Excluded Teeth</h5>
          <button type="button" className="btn-close" onClick={() => toggleModal("excludedTeeth", false)}></button>
        </div>
        <div className="modal-body">
          <p>Excluded teeth editing interface will go here.</p>
          {/* Example structure based on original state names */}
          <div>
            Current section: {selectedExcludedTeethSection}
            {/* Add sidebar and content based on sidebarSections and observationSubObservations */}
          </div>
        </div>
        <div className="modal-footer">
          <Button color="light" onClick={() => toggleModal("excludedTeeth", false)}>Cancel</Button>
          <Button color="primary" onClick={() => toggleModal("excludedTeeth", false) /* Handle save */}>Save</Button>
        </div>
      </Modal>
